---
name: dg-learn
description: Generates a study guide and quiz from any learning material (Markdown / PDF / URL / folders / mixed inputs), supports two verification modes - self-quiz by reading the file, or AI-conducted test where the skill asks questions one by one, scores immediately, and generates a wrong-answer set for re-test. Use when the user says "/dg-learn", "学习这份资料", "出题考我", "测试我对...的理解", or wants structured learning + testing from material. Auto-dispatches subagents for multi-material parallel processing, supports difficulty levels (easy/medium/hard/mixed) and question types (choice/judge/short/apply/mixed). Auto-splits quiz into multiple files when total > 20 questions. Strict scoring (off-topic answers get 0). Does NOT support pause/resume mid-test, real-time tutoring dialogue, or exam features like time limits.
version: 1.0.0
---
...
- ✅ **做**：解析多种资料（md/pdf/url/folder/混合）、派 subagent 并行处理多资料、生成结构化学习指南、生成 4 种题型题库（含拆分）、AI 问答测试（一道一道立刻判 + 错题集生成）、模式自动识别
...
| `/dg-learn quiz-20260626.md` | 模式=测试（资料是题库） |
| `/dg-learn wrong-answers-20260626.md` | 模式=测试（错题重做） |
...
输出：`guide-{ts}.md` + `quiz-{ts}.md`（或 `quiz-{ts}/` 目录）

### 测试模式（test）

输入：题库文件（带 `dg-learn-quiz: true` frontmatter）
输出：测试交互 + `wrong-answers-{ts}.md` 错题集
...
1. **核心知识点去重**：不同资料里讲同一知识点的合并描述
2. **重点/难点综合**：跨资料合并，去掉重复项
3. **关键示例挑选**：选 3-5 个最有代表性的
4. **建议出题点汇总**：用于 Step 6 出题

得到合并后的 `KNOWLEDGE_POINTS[]`（用于学习指南和题库的 `knowledge_point` 字段）。

### Step 5: 生成学习指南

加载 [references/study-guide-template.md](references/study-guide-template.md)，按其模板生成 `guide-{YYYYMMDD-HHmmss}.md`：

- **6 段固定**：资料清单 / 主题概述 / 核心知识点 / 重点 / 难点 / 学习路径
- **3 段按需**：先修知识 / 常见误区 / 延伸阅读（仅在 subagent 摘要里明显能提取到才写）
- **详细程度默认中等**：每个知识点配 1-2 句解释
- **资料清单表格**：按用户输入顺序排，文件夹汇总为一行
- **学习路径**：每份资料一个角色标签（基础/重点/难点/应用/进阶）
- frontmatter 字段：`dg-learn-guide`、`source_materials`、`knowledge_points`、`stats`、`quiz_file`（关联题库路径）

写到用户当前工作目录的 `guide-{ts}.md`。

### Step 6: 生成题库

加载 [references/quiz-template.md](references/quiz-template.md)，按其模板生成题库：

**6a. 决定题目数量：**
- `COUNT != null` → 强制按 COUNT 出题，分配到知识点
- `COUNT == null` → 不限制，每个核心知识点至少 1 题，再按 subagent 的"建议出题点"加码（目标是覆盖所有考点）
...
**6d. 生成题目：**
- 每题的 `knowledge_point` 必须对齐学习指南的核心知识点清单
- 每题的 `source` 关联资料 # 编号
- 主观题（short/apply）必须给 `scoring_points`（让 AI 评分时有明确标准）
- 答案紧跟每题后，用 `<details>` 折叠

**6e. 决定文件结构：**
- 题目总数 ≤ 20 → 单文件 `quiz-{ts}.md`
- 题目总数 > 20 → 拆分目录 `quiz-{ts}/`：
...
📝 题库：quiz-{ts}.md（或 quiz-{ts}/ 目录）
   - 总题数：18（选择 8 / 判断 3 / 简答 5 / 应用 2）
   - 难度：简单 5 / 中等 10 / 困难 3
   
   （如果是拆分目录）
   📁 子题库：
      - quiz-{ts}/quiz-index.md（索引）
      - quiz-{ts}/choice/hooks.md（6 题，Hooks）
      - quiz-{ts}/choice/state.md（6 题，State）
      ...

💡 接下来：
   - 自测：直接打开题库文件查看
   - AI 问答测试：/dg-learn quiz-{ts}.md（或 quiz-{ts}/quiz-index.md）
```
...
3. 错题已生成到 wrong-answers-{ts}.md，可针对性重做
```
...
按 [references/quiz-template.md](references/quiz-template.md) 的"错题集文件"段，生成 `wrong-answers-{ts}.md`：
...
- 提示用户：`/dg-learn wrong-answers-{ts}.md` 可重做错题
...
| [references/design-plan.md](references/design-plan.md) | 完整设计决策记录（迭代时参考） |
