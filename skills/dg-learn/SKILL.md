...
| `/dg-learn quiz.md` | 模式=测试（资料是题库） |
| `/dg-learn wrong-answers.md` | 模式=测试（错题重做） |
...
输出：`guide.md` + `quiz.md`（或 `quiz/` 目录）

### 测试模式（test）

输入：题库文件（带 `dg-learn-quiz: true` frontmatter）
输出：测试交互 + `wrong-answers.md` 错题集
...
📝 题库：quiz.md（或 quiz/ 目录）
   - 总题数：18（选择 8 / 判断 3 / 简答 5 / 应用 2）
   - 难度：简单 5 / 中等 10 / 困难 3
   
   （如果是拆分目录）
   📁 子题库：
      - quiz/quiz-index.md（索引）
      - quiz/choice/hooks.md（6 题，Hooks）
      - quiz/choice/state.md（6 题，State）
      ...

💡 接下来：
   - 自测：直接打开题库文件查看
   - AI 问答测试：/dg-learn quiz.md（或 quiz/quiz-index.md）
```
...
3. 错题已生成到 wrong-answers.md，可针对性重做
```
...
- 提示用户：`/dg-learn wrong-answers.md` 可重做错题
