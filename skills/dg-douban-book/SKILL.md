...
> v2.7.2 起，脚本会自动处理豆瓣的 PoW 反爬挑战（命中 `#sub` 按钮时点击 → 等前端 JS 跑 SHA-512 → 跳回结果页），并把 cookie 持久化到 `/tmp/douban-state.json` 供下次复用——对用户透明，正常情况下无需感知。
...
v2.7.2 起脚本会自动绕过 PoW 反爬挑战 + 复用 `/tmp/douban-state.json` 里的 cookie，触发 `blocked` 的概率已大幅降低。**仍触发**时通常意味着 cookie 失效或频控较严：

按 output-template.md 的 blocked 分支渲染。**不要**自动重试（重试只会再被频控）。建议用户：

1. 在浏览器手动登录 douban.com 一次（任意浏览器，登录态会让下次脚本复用的 cookie 有效）
2. 删除 `/tmp/douban-state.json` 让脚本重新生成（`rm /tmp/douban-state.json`）
3. 等 15–25 分钟让 IP 自然解封后再跑
...
| `status=blocked` 频繁 | ① 检查 `/tmp/douban-state.json` 是否生成（生成说明 cookie 持久化生效）；② 仍频繁 blocked → 浏览器手动登录 douban.com 一次让 cookie 带登录态；③ 删 `/tmp/douban-state.json` 后重跑（`rm /tmp/douban-state.json`）；④ 仍不行 → 等 15–25 分钟让 IP 解封 |
