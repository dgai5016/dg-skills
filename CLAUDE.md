...
| `dg-learn` | 1.2.0 | 对任意资料生成学习指南 + 测试题（4 种题型）→ 学习指南以学习路径为主线（每阶段含目标 / 知识点带章节出处 / 学习提示）→ 产物整合到 `dg-learn/{name}/` 目录（AI 推荐目录名 + 用户选/自定义）→ 支持自测或 AI 问答测试（一道一道立刻判 + 错题集重做）→ 多资料 subagent 并行处理 + 题库超 20 题自动拆分 |
